...
public class BulletSpawner : MonoBehaviour
{
    ...
    [Tooltip("本番の〇判定対象の球が見えてから、何秒後に時間を止めるか")]
    [Min(0f)]
    public float mainCircleChallengeDelayAfterSpawn = 2f;
    [Tooltip("以前の互換用。現在は見えてからの秒数で止めるため使用しません")]
    ...
    {
        if (mainCircleChallengeDelayAfterSpawn > 0f)
        {
            yield return new WaitForSeconds(mainCircleChallengeDelayAfterSpawn);
        }
    ...
    private void SpawnBall(BallScheduleEntry entry, bool isTutorial)
    {
        ...
        var ball = new GameObject($"{prefab.name}_Obstacle");
    ...
    {
        return Mathf.Max(0f, entry.time);
    }
