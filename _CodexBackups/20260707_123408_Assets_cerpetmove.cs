...
public class CarpetMove : MonoBehaviour
{
    ...
    public bool shieldShowVisualEffect = true;
    public bool shieldAlwaysVisible = false;
    public bool shieldLivePreview = false;
    public float shieldDuration = 5f;
    public string shieldVisualObjectName = "ShieldAuraPreview";
    public bool shieldAttachToMainCamera = false;
    public bool shieldForceParticleLocalAlignment = true;
    public Vector3 shieldLocalPosition = new Vector3(0f, 2.0f, 5.0f);
    public Vector3 shieldLocalRotation = new Vector3(0f, 0f, 0f);
    public Vector3 shieldEffectRotationOffset = new Vector3(0f, 0f, 0f);
    public Vector3 shieldLocalScale = new Vector3(1f, 1f, 1f);
    ...
    public bool alignPlayerHitboxToCamera = true;
    public float playerHitboxRadius = 1f;
    [Tooltip("当たった後、モンスターの見た目を残す秒数")]
    public float hitObjectRemainSeconds = 0.8f;
    ...
    void AlignPlayerHitboxToCamera()
    {
        if (!alignPlayerHitboxToCamera)
        {
            return;
        }
        ...
        var targetCamera = cameraObject != null ? cameraObject.transform : Camera.main != null ? Camera.main.transform : null;
        if (targetCamera != null && targetCamera.IsChildOf(transform))
        {
            hitbox.center = targetCamera.localPosition;
        }

        hitbox.radius = Mathf.Max(0.1f, playerHitboxRadius);
    }

    public void ApplyShieldSettings()
    {
        if (shieldController == null)
        {
            return;
        ...
        shieldController.showVisualEffect = shieldShowVisualEffect;
        shieldController.alwaysVisible = shieldAlwaysVisible;
        shieldController.shieldDuration = shieldDuration;
        shieldController.sceneVisualEffectName = shieldVisualObjectName;
        shieldController.attachToMainCamera = shieldAttachToMainCamera;
        shieldController.forceParticleLocalAlignment = shieldForceParticleLocalAlignment;
        shieldController.localPosition = shieldLocalPosition;
        shieldController.localRotation = shieldLocalRotation;
        shieldController.effectRotationOffset = shieldEffectRotationOffset;
        shieldController.localScale = shieldLocalScale;
        shieldController.ApplyTransform();
    }
